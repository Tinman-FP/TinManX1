...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    if(!has_BTT_thumbnail){   
        ...
        const std::string timestamp = Slic3r::Utils::local_timestamp();
        const std::string tinman_generator = Slic3r::header_slic3r_generated();
        file.write_format("; generated by %s on %s\n", tinman_generator.c_str(), timestamp.c_str());

        // Older Moonraker/Snapmaker metadata extractors only recognize
        // Prusa/Orca-style slicer names. Keep TinmanX1 as the real generator,
        // but provide a compatibility identity so the printer UI can parse
        // slicer metadata such as estimated print time.
        std::string compat_version = tinman_generator;
        const size_t version_pos = compat_version.find_last_of(' ');
        if (version_pos != std::string::npos && version_pos + 1 < compat_version.size())
            compat_version = compat_version.substr(version_pos + 1);
        file.write_format("; generated by OrcaSlicer %s on %s\n", compat_version.c_str(), timestamp.c_str());
        // Keep the print-time metadata close to the top of the file so older
        // Moonraker/Qidi builds can index it without scanning the footer.
    ...
    // Fork-specific diagnostics must remain outside HEADER_BLOCK. Several printer
    // firmware parsers reject unknown header keys even though they are comments.
    ...
    for (size_t region_idx = 0; region_idx < print.num_print_regions(); ++region_idx) {
        ...
        if (region_config.wave_overhangs.value && region_config.wave_overhang_debug_gcode.value) {
            any_wave_debug = true;
            break;
        }
    }
    if (any_wave_debug) {
        file.write_format("; WAVE_OVERHANG_BUILD source=dennisklappe/OrcaSlicer-WaveOverhangs tag=v0.4.0 rev=f6a901d57cd128c922c81591ceae4fd0b7cc5524\n");
        for (size_t region_idx = 0; region_idx < print.num_print_regions(); ++region_idx) {
            const PrintRegionConfig &config = print.get_print_region(region_idx).config();
            if (!config.wave_overhangs.value || !config.wave_overhang_debug_gcode.value)
                continue;
            file.write_format(
                "; WAVE_OVERHANG_CONFIG region=%zu algorithm=wavefront support_remainder=%d instead_of_bridges=%d"
                " spacing=%.3f flow_mm3_per_mm=%.3f speed=%.1f travel=%.1f fan=%d aux_fan=%d"
                " floor_layers=%d floor_speed=%.1f floor_wall_speed=%.1f floor_speed_ramp=%d"
                " floor_fan=%d floor_aux_fan=%d\n",
                region_idx,
                config.support_remaining_areas_after_wave_overhangs.value ? 1 : 0,
                config.wave_overhangs_instead_of_bridges.value ? 1 : 0,
                config.wave_overhang_line_spacing.value,
                config.wave_overhang_flow_mm3_per_mm.value,
                config.wave_overhang_print_speed.value,
                config.wave_overhang_travel_speed.value,
                config.wave_overhang_fan_speed.value,
                config.wave_overhang_aux_fan_speed.value,
                config.wave_overhang_floor_layers.value,
                config.wave_overhang_floor_print_speed.value,
                config.wave_overhang_floor_perimeter_speed.value,
                config.wave_overhang_floor_speed_ramp.value,
                config.wave_overhang_floor_fan_speed.value,
                config.wave_overhang_floor_aux_fan_speed.value);
        }
    }
    }
    ...
    const bool                               prime_extruder)
{
    ...
    for (const LayerToPrint &layer_to_print : layers) {
        if (layer_to_print.object_layer == nullptr)
            continue;
        for (const std::string &diagnostic : layer_to_print.object_layer->wave_overhang_diagnostics)
            gcode += diagnostic;
    }
    ...
    if (m_wave_layer_accumulated_time > 0.) {
        const double minimum_time = m_config.wave_overhang_min_layer_time.value;
        if (minimum_time > 0. && m_wave_layer_accumulated_time < minimum_time)
            gcode += Slic3r::format("G4 P%d ; wave-overhang min_layer_time dwell\n",
                int((minimum_time - m_wave_layer_accumulated_time) * 1000. + 0.5));
...
std::string GCode::_extrude(const ExtrusionPath &path, std::string description, double speed)
{
    ...
    const int floor_main_fan = m_config.wave_overhang_floor_fan_speed.value;
    const int floor_aux_fan = m_config.wave_overhang_floor_aux_fan_speed.value;
    const bool wave_fan_active = path.wave_overhang
        && (wave_main_fan >= 0 || wave_aux_fan >= 0) && m_enable_cooling_markers;
    const bool floor_fan_active = path.wave_overhang_floor
        && (floor_main_fan >= 0 || floor_aux_fan >= 0) && m_enable_cooling_markers;

    if (path.wave_overhang)
        m_inside_wave_overhang = true;
    if (wave_fan_active)
        gcode += Slic3r::format(";_WAVE_OVERHANG_FAN_START %d %d\n", wave_main_fan, wave_aux_fan);
    else if (floor_fan_active)
        gcode += Slic3r::format(";_WAVE_OVERHANG_FAN_START %d %d\n", floor_main_fan, floor_aux_fan);
    ...
    const int restore_nozzle_temp = m_writer.filament() == nullptr ? 0
        : (this->on_first_layer()
    ...
    if (wave_temp_active)
        gcode += Slic3r::format("M104 S%d ; wave-overhang temp override\n", wave_nozzle_temp);
    ...
    // ORCA: resonance‑avoidance on short external perimeters
{
    ...
    if (wave_fan_active || floor_fan_active)
        gcode += ";_WAVE_OVERHANG_FAN_END\n";
    if (wave_temp_active && restore_nozzle_temp > 0)
        gcode += Slic3r::format("M104 S%d ; wave-overhang temp restore\n", restore_nozzle_temp);
    if (path.wave_overhang)
        m_inside_wave_overhang = false;

    if (path.wave_overhang && m_config.wave_overhang_end_retract_length.value > 0.)
        gcode += m_writer.retract(false, m_config.wave_overhang_end_retract_length.value);
    if (emit_wave_overhang_markers)
        gcode += "; WAVE_OVERHANG_END\n";

    if (path.wave_overhang) {
        const double wave_speed = std::max(0.01, m_config.wave_overhang_print_speed.value);
        const double region_time = unscale<double>(path.length()) / wave_speed;
        m_wave_layer_accumulated_time += region_time;
        const double minimum_time = m_config.wave_overhang_min_wave_time.value;
        if (minimum_time > 0. && region_time < minimum_time)
            gcode += Slic3r::format("G4 P%d ; wave-overhang min_wave_time dwell\n",
                int((minimum_time - region_time) * 1000. + 0.5));
    }
    return gcode;
}
...
std::string GCode::travel_to(const Point& point, ExtrusionRole role, std::string comment, double z/* = DBL_MAX*/)
{
    ...
    const double wave_travel_speed = m_inside_wave_overhang && m_config.wave_overhang_travel_speed.value > 0.
        ? m_config.wave_overhang_travel_speed.value : 0.;
