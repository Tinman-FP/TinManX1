...
PASS_THROUGH_STATUSES = {
    "failed",
    "skipped_no_arc_moves",
    "skipped_no_source_geometry",
}
STRICT_ARC_SUPPORT = os.environ.get(
    "ORCASLICER_CODEX_ARC_SUPPORT_STRICT",
    os.environ.get("TINMANX_ARC_SUPPORT_STRICT", "0"),
) in {"1", "true", "True", "yes"}
...
def _prepend_preview_metadata(
    ...
    *,
    ...
    output_path.write_text("".join(_insert_postprocess_metadata(lines, metadata)), encoding="utf-8")


def _write_passthrough_output(output_path: Path, before_lines: list[str], status: str, error: str | None) -> None:
    existing_prefixes = ("; orcaslicer_codex_arc_support_", "; tinmanx_arc_support_")
    clean_lines = [line for line in before_lines if not line.startswith(existing_prefixes)]
    metadata = [
        f"; orcaslicer_codex_arc_support_status={status}\n",
        f"; orcaslicer_codex_arc_support_transform_status={status}\n",
        "; orcaslicer_codex_arc_support_strategy=arc\n",
        "; orcaslicer_codex_arc_support_base_path=bridge_overhang_generation\n",
        "; orcaslicer_codex_arc_support_postprocess_stage=passthrough_original_gcode\n",
        "; orcaslicer_codex_arc_support_guarded_transform_required=false\n",
        "; orcaslicer_codex_arc_support_preview_note=arc_transform_unavailable_original_gcode_preserved\n",
    ]
    if error:
        metadata.append(f"; orcaslicer_codex_arc_support_reason={error.replace(chr(10), ' ')}\n")
    output_path.write_text("".join(_insert_postprocess_metadata(clean_lines, metadata)), encoding="utf-8")


def _insert_postprocess_metadata(lines: list[str], metadata: list[str]) -> list[str]:
    """Keep slicer metadata as the first G-code header for strict printer parsers."""
    for index, line in enumerate(lines):
        if line.startswith("; HEADER_BLOCK_END"):
            insert_at = index + 1
            spacer = [] if insert_at < len(lines) and lines[insert_at].strip() == "" else ["\n"]
            return lines[:insert_at] + spacer + metadata + lines[insert_at:]
...
def transform_gcode(input_path: Path, output_path: Path, audit_path: Path, engine_path: Path) -> int:
    ...
    if status == "ok" and after["arc_infill_extrusion_moves"] == 0:
        if before["bridge_type_markers"] > 0 or before["normal_support_type_markers"] > 0:
            ...
            status = "skipped_no_source_geometry"
            error = "Arc Overhang transform found no bridge or support source geometry; original G-code was preserved."
    coverage_ratio = 1.0
    ...
    return 0 if status == "ok" or status.startswith(FALLBACK_PREFIX) else 2
